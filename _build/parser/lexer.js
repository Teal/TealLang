...
var Lexer = (function () {
    ...
    Lexer.prototype.scan = function () {
        ...
        while (true) {
            ...
            switch (ch) {
                    ...
                    switch (this.source.charCodeAt(this.pos)) {
                        case 61 /* equals */:
                            ...
                            if (this.source.charCodeAt(this.pos) === 61 /* equals */) {
                                this.pos++; // =
                                if (compiler_1.options.allowGitConflictMarker !== false && result.onNewLine && this.skipGitConflictMarker(61 /* equals */, 4)) {
                                    // 跳过冲突的第二个版本。
                                    while (this.pos < this.source.length) {
                ...
                case 96 /* backtick */:
                    result.data = this.scanStringLiteral(ch);
                    result.type = this.source.charCodeAt(this.pos - 1) === 123 /* openBrace */ ? tokenType_1.TokenType.templateHead : tokenType_1.TokenType.noSubstitutionTemplateLiteral;
                    break;
                // +、++、+=
                case 43 /* plus */:
                    switch (this.source.charCodeAt(this.pos)) {
                ...
                case 60 /* lessThan */:
                    switch (this.source.charCodeAt(this.pos)) {
                        case 60 /* lessThan */:
                            this.pos++; // <
                            if (compiler_1.options.allowGitConflictMarker !== false && result.onNewLine && this.skipGitConflictMarker(60 /* lessThan */, 5)) {
                                continue;
                            }
                            if (this.source.charCodeAt(this.pos) === 61 /* equals */) {
                                this.pos++; // =
    ...
    Lexer.prototype.readAsTemplateMiddleOrTail = function () {
        ...
        return this.current = this.current._next = {
            start: this.current.start,
            type: this.source.charCodeAt(this.pos - 1) === 123 /* openBrace */ ? tokenType_1.TokenType.templateMiddle : tokenType_1.TokenType.templateTail,
            data: data,
            end: this.pos
        };
    };
